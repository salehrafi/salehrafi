(() => {
  ...
  document.querySelectorAll('.pill-row--interactive .pill[data-level]').forEach(pill => {
    pill.setAttribute('title', `Proficiency: ${pill.getAttribute('data-level')}%`);
  });
  ...
  if (form) {
    ...
    form.addEventListener('submit', (e) => {
      ...
      // Simulated send — replace with a real endpoint (e.g. Formspree, EmailJS)
      // when connecting the form to a backend.
      setTimeout(() => {
  ...
  // EDIT: swap these words for ones that fit your own field/headline
  ...
  if (canvas && !prefersReducedMotion) {
    ...
    function initNodes() {
      ...
      const count = Math.min(70, Math.max(24, Math.round((area / 1000000) * NODE_COUNT_BASE)));
